...
class RuntimeInstallTest(unittest.TestCase):
    def test_manual_package_contains_scripts_and_configs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(docker_stop.exists())
            self.assertTrue(docker_stop.stat().st_mode & stat.S_IXUSR)
            ...
            self.assertEqual(categories["achost/bin/achost-docker-stop.sh"], "docker")
            ...
            self.assertIn('exec "$ACHOST_DOCKER_RUNTIME" start "$@"', docker_start_text)
            self.assertNotIn("wait_for_bridge", docker_start_text)
            self.assertNotIn("network reconciled bridge=", docker_start_text)
            self.assertNotIn("mount_chroot_memory_cgroup_v1", docker_start_text)
            self.assertNotIn("cleanup_stale_docker_iptables", docker_start_text)
            docker_stop_text = docker_stop.read_text()
            self.assertIn('exec "$ACHOST_DOCKER_RUNTIME" stop "$@"', docker_stop_text)
            self.assertNotIn("stop_pid_file", docker_stop_text)
            self.assertNotIn("pidof", docker_stop_text)
    ...
    def test_kernelsu_module_contains_module_entrypoints(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("achost-docker-stop.sh", uninstall_text)
    ...
    def test_native_runtime_mode_writes_config_and_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('exec "$ACHOST_DOCKER_RUNTIME" start "$@"', docker_start)
            self.assertNotIn("native_preflight()", docker_start)
            self.assertNotIn("setup_native_root_files()", docker_start)
            self.assertNotIn("daemon_namespace_diagnostics()", docker_start)
            self.assertNotIn("setup_devices_cgroup()", docker_start)
            self.assertNotIn("ensure_host_memory_cgroup()", docker_start)
            self.assertNotIn("write_dockerd_config()", docker_start)
            self.assertNotIn("write_containerd_config()", docker_start)
            ...
            self.assertNotIn("bind_chroot_path", docker_start)
    ...
    def test_cgroup_v2_mode_writes_runtime_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('exec "$ACHOST_DOCKER_RUNTIME" start "$@"', docker_start)
            self.assertNotIn("setup_chroot_cgroups_v2", docker_start)
            self.assertNotIn("mount_chroot_memory_cgroup_v1", docker_start)

...
class RuntimeInstallTest(unittest.TestCase):
    def test_manual_package_contains_scripts_and_configs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("prepare-cgroups", docker_start_text)
            self.assertNotIn("ensure_host_memory_cgroup()", docker_start_text)
            ...
            self.assertIn("cleanup-stale-iptables", docker_start_text)
    ...
    def test_native_runtime_mode_writes_config_and_manifest(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("prepare-native-root", docker_start)
            self.assertIn("prepare-cgroups", docker_start)
            self.assertIn("native-preflight", docker_start)
            self.assertIn("prepare-compat-socket", docker_start)
            self.assertIn("write-configs", docker_start)
            self.assertIn("namespace-diagnostics", docker_start)

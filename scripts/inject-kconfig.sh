...
usage() {
    printf 'Usage: %s --kernel-tree /path/to/kernel [--destination PATH] [--apply] [--json]\n' "${0##*/}"
}
...
CMD=(python3 -m achost.cli inject-kconfig --kernel-tree "$KERNEL_TREE" --destination "$DESTINATION")
if [[ "$APPLY" -eq 1 ]]; then
    CMD+=(--apply)
fi
if [[ "$JSON" -eq 1 ]]; then

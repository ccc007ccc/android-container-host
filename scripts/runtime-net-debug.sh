...
run_shell "detected uplink" "if [ -x '$SCRIPT_DIR/detect-uplink.sh' ]; then '$SCRIPT_DIR/detect-uplink.sh' 1.1.1.1; else ip route get 1.1.1.1; fi"
run_shell "network watchdog status" "if [ -r '$ACHOST_NET_PID' ]; then pid=\$(cat '$ACHOST_NET_PID' 2>/dev/null); printf 'pid=%s\\n' \"\$pid\"; if kill -0 \"\$pid\" 2>/dev/null; then printf 'running=1\\n'; else printf 'running=0\\n'; fi; else printf 'pid file not found: %s\\n' '$ACHOST_NET_PID'; fi; { ps -A 2>/dev/null || ps 2>/dev/null; } | grep '[c]ontainer-network-watchdog' || true"

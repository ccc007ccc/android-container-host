...
```bash
scripts/package-all.sh --version 0.1.2
```

输出：

```text
out/achost-v0.1.2/achost-base-v0.1.2.zip
out/achost-v0.1.2/achost-docker-v0.1.2.zip
out/achost-v0.1.2/achost-lxc-v0.1.2.zip
out/achost-v0.1.2/SHA256SUMS.txt
```
...
```bash
scripts/package-base.sh --version 0.1.2
scripts/package-docker.sh --version 0.1.2
scripts/package-lxc.sh --version 0.1.2
```
...
```bash
scripts/package-docker.sh --version 0.1.2 --clean-output
```
...
```bash
scripts/package-all.sh --version 0.1.2 --assets-dir /path/to/assets
```
...
```bash
scripts/package-docker.sh --version 0.1.2 --dry-run
```
...
```bash
unzip -l out/achost-v0.1.2/achost-docker-v0.1.2.zip \
  | grep -E 'docker-compose|docker-buildx|buildctl|buildkitd'

unzip -l out/achost-v0.1.2/achost-lxc-v0.1.2.zip \
  | grep 'lxc-download'

cd out/achost-v0.1.2
...
PYTHONPATH=$PWD python3 -m achost.cli runtime-validate \
  ...
  --package-root out/achost-v0.1.2/achost-docker \
  --zip out/achost-v0.1.2/achost-docker-v0.1.2.zip \
  --release
```
...
```bash
scripts/package-all.sh --version 0.1.2 --clean-output
```

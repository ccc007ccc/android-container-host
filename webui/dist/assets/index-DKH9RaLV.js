(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))l(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const p of i.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&l(p)}).observe(document,{childList:!0,subtree:!0});function a(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function l(n){if(n.ep)return;n.ep=!0;const i=a(n);fetch(n.href,i)}})();let Z=0;function j(t){return`${t}_callback_${Date.now()}_${Z++}`}function M(t,e){return typeof e>"u"&&(e={}),new Promise((a,l)=>{const n=j("exec");window[n]=(p,T,B)=>{a({errno:p,stdout:T,stderr:B}),i(n)};function i(p){delete window[p]}try{ksu.exec(t,JSON.stringify(e),n)}catch(p){l(p),i(n)}})}function U(t){ksu.enableEdgeToEdge(t)}function R(t){ksu.toast(t)}const F="/data/adb/modules/achost-runtime/achost/bin/achost-webui-api.sh",c=document.querySelector("#app");let S=F,f="dashboard",s=null,k=[],y=[],b="",o="",E="",L="",A="",_="",q="",D="",C="bridge",I="";try{U(!0)}catch{}function r(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function H(t){return`'${t.replace(/'/g,"'\\''")}'`}function J(t){return/^[A-Za-z0-9_.-]+$/.test(t)}function x(t){return/^[A-Za-z0-9_./:@-]+$/.test(t)}function w(t,e){return!t.trim()||t.split(",").every(a=>e.test(a.trim()))}function g(t){try{R(t)}catch{}}async function K(){try{const t=await fetch("./achost-webui-config.json",{cache:"no-store"});if(!t.ok)return;const e=await t.json();typeof e.api=="string"&&e.api.startsWith("/data/adb/modules/")&&(S=e.api)}catch{}}async function h(t,e=[]){const a=[S,t,...e].map(H).join(" "),l=await M(a),n=l.stdout.trim()||l.stderr.trim();if(!n)return{ok:!1,error:`命令没有输出，errno=${l.errno}`};try{const i=JSON.parse(n);return l.errno!==0&&i.ok!==!1&&(i.ok=!1,i.error=`errno=${l.errno}`),i}catch{return{ok:!1,error:n}}}function O(t){return typeof t.output=="string"?t.output:typeof t.error=="string"?t.error:JSON.stringify(t,null,2)}async function P(){o="refresh",v();try{const[t,e,a]=await Promise.all([h("status"),h("list-containers"),h("list-images")]);s=t,k=Array.isArray(e.containers)?e.containers:[],y=Array.isArray(a.images)?a.images:[],t.ok===!1&&(b=O(t))}catch(t){b=t instanceof Error?t.message:String(t),g("刷新失败")}finally{o="",v()}}async function d(t,e,a=[],l=!0){o=e,v();try{const n=await h(e,a);b=O(n),g(n.ok===!1?`${t}失败`:`${t}完成`),l&&await P()}catch(n){b=n instanceof Error?n.message:String(n),g(`${t}失败`)}finally{o="",v()}}async function W(){const t=L.trim(),e=A.trim(),a=_.trim(),l=q.trim(),n=D.trim(),i=C.trim()||"bridge";if(!J(t)){g("容器名只能包含字母、数字、点、下划线和短横线");return}if(!x(e)){g("镜像名包含不支持的字符");return}if(!w(a,/^[0-9:/a-z.-]+$/)){g("端口映射格式不合法");return}if(!w(l,/^[A-Za-z_][A-Za-z0-9_=@.,:/+-]*$/)){g("环境变量格式不合法");return}if(!w(n,/^\/[A-Za-z0-9_./:@,+=-]+:\/[A-Za-z0-9_./:@,+=-]+$/)){g("挂载格式不合法");return}await d("创建容器","add-container",[t,e,a,l,n,i])}async function G(){const t=I.trim();if(!x(t)){g("镜像名包含不支持的字符");return}await d("拉取镜像","pull-image",[t])}async function Q(t){await d(t?"开启自启":"关闭自启","set-autostart",[t?"on":"off"])}async function Y(){f="diagnostics",await d("运行检查","check",[],!1)}function V(t){return t.status.toLowerCase().startsWith("up")}function z(){const t=!!s?.running;return`<span class="pill ${t?"ok":"stop"}"><span></span>${t?"Docker 运行中":"Docker 已停止"}</span>`}function $(t,e,a=""){return`<section class="card stat ${a}"><span>${t}</span><strong>${r(e)}</strong></section>`}function u(t,e){return`<div class="detail-item"><span>${t}</span><strong>${r(e||"—")}</strong></div>`}function m(t,e){return`<button class="nav-item ${f===t?"active":""}" data-tab="${t}">${e}</button>`}function X(){return`
  ...
  `}function tt(){const t=E.trim().toLowerCase();return t?k.filter(e=>[e.name,e.id,e.image,e.status].some(a=>a.toLowerCase().includes(t))):k}function et(){const t=tt();return t.length?t.map(e=>{const a=e.name||e.id,l=V(e);return`
        ...
        </article>`}).join(""):'<div class="empty">没有匹配的容器。可以用下方表单创建 detached 容器。</div>'}function at(){return`
    <section class="panel">
  ...
  `}function N(t){return t.repository==="<none>"||t.tag==="<none>"?t.id:`${t.repository}:${t.tag}`}function nt(){const t=y.length?y.map(e=>`
          <article class="table-row image-row">
            ...
            <div class="row-actions">
              <button class="small danger" data-remove-image="${r(N(e))}" ${o?"disabled":""}>删除镜像</button>
            </div>
  ...
  `}function it(){return f==="containers"?at():f==="images"?nt():f==="diagnostics"?rt():f==="settings"?st():X()}function v(){c.innerHTML=`
